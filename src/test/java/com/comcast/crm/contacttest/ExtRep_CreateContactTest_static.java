...
public class ExtRep_CreateContactTest_static extends BaseClass {
...
//..............HOW TO CREATE LOW LEVEL REPORTS WITHIN EXTENT REPORTS?......//
	...
	public void createNewContTest() throws Throwable, Throwable {
		...
		UtilityClassObject.getTest().log(Status.INFO, "Contact Name generated is : " + LastName);
		

		Thread.sleep(2000);

		UtilityClassObject.getTest().log(Status.INFO, "Click on Contacts from homePage");
		...
		UtilityClassObject.getTest().log(Status.INFO,"Enter the contact details and save the contact");
		...
		if (headerName.contains(LastName)) {
			UtilityClassObject.getTest().log(Status.INFO,"Contact detail is driven == PASS");
		} else {
			UtilityClassObject.getTest().log(Status.INFO,"Contact detail is not driven == FAIL");
		}

		// Verify cont last name
		String SavedLastName = contInfoPOM.getContLastNameInfo().getText();
		if (SavedLastName.equals(LastName)) {
			UtilityClassObject.getTest().log(Status.INFO,"Contact is created and verified == PASS");
		} else {
			UtilityClassObject.getTest().log(Status.INFO,"Contact is not verified== FAIL");
		}

		// Step 5: Logout of the Application
		UtilityClassObject.getTest().log(Status.INFO,"createNewContTest: EXECUTION SUCCESS");
	}
