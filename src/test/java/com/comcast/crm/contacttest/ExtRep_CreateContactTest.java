...
import com.comcast.crm.ListenerUtility.ListenerImpWithExtentReportConfig;
import com.comcast.crm.generic.ExcelUtility.ExcelUtility;
import com.comcast.crm.generic.FileUtility.FileUtility;
import com.comcast.crm.generic.WebDriverUtility.JavaUtility;
import com.comcast.crm.generic.WebDriverUtility.WebDriverUtility;

public class ExtRep_CreateContactTest extends BaseClass {


	// TestCase : Create New Contact
	
	
//..............HOW TO CREATE LOW LEVEL REPORTS WITHIN EXTENT REPORTS?......//
	//......Make the test variable public & static in the ListenerImplemetationclass
	//......Use that variable here as ListImpl.test
	...
	public void createNewContTest() throws Throwable, Throwable {
		...
		ListenerImpWithExtentReportConfig.test.log(Status.INFO, "Read Data from Excel");
		String LastName = elib.getDataFromExcel("contact", 1, 2) + ranNum;
		ListenerImpWithExtentReportConfig.test.log(Status.INFO, "Contact Name generated is : " + LastName);
		

		Thread.sleep(2000);

		ListenerImpWithExtentReportConfig.test.log(Status.INFO, "Click on Contacts from homePage");
		HomePage hpPOM = new HomePage(driver);
		hpPOM.getConLink().click();
		
		ListenerImpWithExtentReportConfig.test.log(Status.INFO,"Click on create contact plus icon");
		ContactHomePage contHpPOM = new ContactHomePage(driver);
		contHpPOM.getCreateNewContPlusIcon().click();
		
		ListenerImpWithExtentReportConfig.test.log(Status.INFO,"Enter the contact details and save the contact");
		...
		if (headerName.contains(LastName)) {
			ListenerImpWithExtentReportConfig.test.log(Status.INFO,"Contact detail is driven == PASS");
		} else {
			ListenerImpWithExtentReportConfig.test.log(Status.INFO,"Contact detail is not driven == FAIL");
		}

		// Verify cont last name
		String SavedLastName = contInfoPOM.getContLastNameInfo().getText();
		if (SavedLastName.equals(LastName)) {
			ListenerImpWithExtentReportConfig.test.log(Status.INFO,"Contact is created and verified == PASS");
		} else {
			ListenerImpWithExtentReportConfig.test.log(Status.INFO,"Contact is not verified== FAIL");
		}

		// Step 5: Logout of the Application
		ListenerImpWithExtentReportConfig.test.log(Status.INFO,"createNewContTest: EXECUTION SUCCESS");
	}
